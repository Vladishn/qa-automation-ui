"""Partner TV application endpoints."""
...
from ..schemas import Platform, TestRunCreate, TestRunSummary, VersionCreate, VersionUnderTest
from ..storage import storage

router = APIRouter(prefix="/app", tags=["applications"])
...
def list_platforms() -> List[Platform]:
    return [
        ...
        for p in storage.list_platforms("APP")
    ]
...
def list_versions(platform_id: Optional[str] = Query(None), channel: Optional[str] = Query(None)) -> List[VersionUnderTest]:
    versions = storage.list_versions("APP", platform_id=platform_id, channel=channel)  # type: ignore[arg-type]
...
def create_version(payload: VersionCreate) -> VersionUnderTest:
    if payload.domain != "APP":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Domain must be APP")
